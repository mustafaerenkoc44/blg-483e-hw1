...
\subsection{Quiz Compatibility Layer}

After the main crawler was completed, the project was additionally aligned with a course quiz that expected two concrete interfaces:

\begin{itemize}
    \item a raw storage file at \texttt{data/storage/p.data},
    \item a search endpoint exposed as \texttt{GET /search?query=... \& sortBy=relevance} on port \texttt{3600}.
\end{itemize}

The core system still stores its authoritative index in SQLite. Therefore, instead of replacing the primary architecture, a compatibility export was added that materializes flat-file rows directly from the canonical index in the following format:

\begin{Verbatim}[fontsize=\small]
word url origin depth frequency
\end{Verbatim}

In parallel, a quiz-specific search route was introduced so that the returned score matches the quiz formula exactly:

\begin{Verbatim}[fontsize=\small]
score = (frequency * 10) + 1000 - (depth * 5)
\end{Verbatim}

This compatibility layer is intentionally separate from the richer dashboard search API. As a result, the original homework design remains intact while the repository also satisfies the quiz workflow exactly.
...
\hline
\texttt{/search?query=... \& sortBy=relevance} & GET & Quiz-compatible search using the exported flat-file scoring formula \\
\hline
\end{tabular}
\label{tab:api}
\end{table}
...
\begin{enumerate}
    ...
    \item quiz-compatible raw export and exact relevance-score calculation.
\end{enumerate}
...
\begin{itemize}
    ...
    \item the dashboard and API were served at \url{http://127.0.0.1:3600},
    ...
    \item the compatibility export file \texttt{data/storage/p.data} was generated successfully,
    \item the compatibility query \texttt{/search?query=relevance\&sortBy=relevance} returned \texttt{page-b.html} with \texttt{relevance\_score = 1025}.
\end{itemize}
...
\subsection{Quiz Validation Example}

The quiz compatibility path was also verified directly against the exported flat file. The chosen word was \texttt{relevance}, which appeared on three distinct URLs:

\begin{Verbatim}[fontsize=\small]
relevance http://127.0.0.1:9001/index.html  http://127.0.0.1:9001/index.html 0 1
relevance http://127.0.0.1:9001/page-a.html http://127.0.0.1:9001/index.html 1 1
relevance http://127.0.0.1:9001/page-b.html http://127.0.0.1:9001/index.html 1 3
\end{Verbatim}

Applying the quiz formula gives:

\begin{itemize}
    \item \texttt{index.html}: $(1 \times 10) + 1000 - (0 \times 5) = 1010$
    \item \texttt{page-a.html}: $(1 \times 10) + 1000 - (1 \times 5) = 1005$
    \item \texttt{page-b.html}: $(3 \times 10) + 1000 - (1 \times 5) = 1025$
\end{itemize}

The top API result from \texttt{GET /search?query=relevance\&sortBy=relevance} was \texttt{page-b.html} with \texttt{relevance\_score = 1025}, which matches the manual calculation exactly.
